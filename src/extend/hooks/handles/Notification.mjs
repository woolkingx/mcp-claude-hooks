// Notification.mjs — Handle Notification hook events
...
export function handle(payload) {
  ...
  const hso = { hookEventName: 'Notification' }
  ...
  log('debug', `notification type=${event.notification_type} title=${event.title || 'none'}`)
